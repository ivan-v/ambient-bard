...
pub static METER_TO_SPACE_VALUES: phf::Map<&'static str, f32> 
    = phf_map! ["4" => 1.0, "8" => 0.5];

pub static RHYTHM_PDF_PRESETS: 
phf::Map<&'static str, phf::Map<&'static str, f32>> = phf_map![
...
fn generate_rhythm_measure(
    space_left: f32, rhythm_pdf: &phf::Map<&'static str, f32>, 
    meter: [f32; 2], given_downbeats: Option<Vec<f32>>)
     -> Vec<String> {
    let mut measure: Vec<String> = Vec::new();
    let num_downbeats 
        = METER_TO_SPACE_VALUES[&meter[1].to_string()] * meter[0];
    let downbeat_after 
        = METER_TO_SPACE_VALUES[&meter[1].to_string()];
    let downbeats: Vec<f32>;
    if given_downbeats != None {
        downbeats = given_downbeats.unwrap_or_default();
    } else {
        downbeats = (0..num_downbeats as i32)
        .map(|x: i32| (x as f32) / (1.0 / downbeat_after as f32)).collect();
    }
    let pdf: Vec<f32> = rhythm_pdf.values().cloned().collect();
    let pmf: Vec<f32> = pdf.iter()
        .scan(0.0, |acc: &mut f32, x: &f32| {
        *acc += x;
        Some(*acc)
    }).collect();

    let mut space_left: f32 = space_left;

    while space_left > 0.0 {
        let r: f32 = rand::thread_rng().gen_range(0..100) as f32 / 100.0;
        let p: &f32 = pmf.iter().find(|&&x| x > r).unwrap();
        let note: &&str = rhythm_pdf.keys()
            .nth(pmf.iter().position(|&x| x == *p).unwrap()).unwrap();
        // making sure that the note doesn't (start and end) on non-downbeat
        if downbeats.contains(&(space_left - SPACE_VALUES[note])) 
...
pub fn generate_rhythm(
    meter: [f32; 2], measure_count: i32, 
    rhythm_pdf: &phf::Map<&'static str, f32>)
     -> Vec<Vec<String>> {
    let breathing_space: bool = check_space(measure_count);
    let mut rhythm:Vec<Vec<String>> = Vec::new();
    let first_measure: Vec<String> = generate_rhythm_measure(
        (meter[0] as f32 / (meter[1] as f32 / 4.0)).ceil() as f32, 
        &rhythm_pdf, meter, None);
    let mut unique_measures: HashSet<Vec<String>> = HashSet::new();
    unique_measures.insert(first_measure.clone());
    let mut measures_left: i32 = measure_count - 1;
    rhythm.append(&mut vec![first_measure]);
    while measures_left > 0 {
        if breathing_space && rand::thread_rng().gen_range(0..=2) == 0 {
            let mut rng: rand::rngs::ThreadRng = rand::thread_rng();
            let random_measure: Vec<String> = (&unique_measures.iter()
                .map(|v: &Vec<String>| v.iter().cloned()
                .collect::<Vec<String>>()).collect::<Vec<Vec<String>>>())
                .choose(&mut rng).unwrap().clone();
            rhythm.append(&mut vec![random_measure.clone()]);
        } else {
            let new_measure: Vec<String> = generate_rhythm_measure(
                (meter[0] as f32 / (meter[1] as f32 / 4.0)).ceil()
                 as f32, &rhythm_pdf, meter, None);
